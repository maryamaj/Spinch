...
-(void) search 
{
    [self.browser searchForServicesOfType:@"_Spinch._tcp." inDomain:@""];
}

#pragma mark SpinchActionPassing Protocol Delegate

-(void) willSendMessageOfType:(int)messageType withIntValue:(int)intValue floatValue:(float)floatValue
{
    
    SpinchMessage *newMessage = [SpinchMessage messageOfType:messageType withIntValue:intValue floatValue:floatValue];
