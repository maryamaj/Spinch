...
-(void) startService
{
    ...
    NSString *serviceName = [NSString stringWithFormat:@"Spinch on %@", [[NSProcessInfo processInfo] hostName]];
    _netService = [[NSNetService alloc] initWithDomain:@"" type:@"_Spinch._tcp." name:serviceName port:self.listeningSocket.localPort];
...
-(void)messageBroker:(TBMessageBroker *)server didReceiveMessage:(SpinchMessage *)message{
    ...
    if ( (_delegate && [_delegate respondsToSelector:@selector(didReceiveMessageOfType: withIntValue: floatValue:)])) {
        
        [_delegate didReceiveMessageOfType:message.messageType withIntValue:message.intValue floatValue:message.floatValue];
